	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		*/
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			 ...
			 * ex)hello.jpg, hello.jpg,hello2.jpg......
			 * 
			 * 해당 rename작업을 따로 정의하여 사용해볼것
			 * rename메소드를 정의하면 해당 작업을 할 수 있다. (내 방식대로)
			 * 
			 * */
			...
			String boardWriter = multiRequest.getParameter("userNo"); //int userNo로 하는거 x
			...
			//첨부파일이 있을 경우 원본명 반환 없는 경우 null을 반환
			...
			if(result>0) {
				...
				//실패시에는 업로드된 파일을 지워주는 작업이 필요하다(게시글은 없는데 업로드파일이 자원을 쓰고 있으니)
				if(at!=null) { //넘어온 파일이 있어서 객체가 생성됐다면.
					//해당 파일 경로 잡아서 File 객체 생성 후 delete메소드로 파일 삭제 작업
